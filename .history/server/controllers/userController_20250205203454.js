...
export const getNews = async (req,res) => {
  console.log('getNews triggering')
  try {
...
export const searchNews = async (req) => {
  try {
    const { query } = req.query;
    const news = await newsService.getNews();
    const filteredNews = newsService.searchNews(query);
    res.json(filteredNews);
  } catch (error) {
    console.error("Error in /news/search route:", error);
